...
module ibex_counter #(
  ...
  assign counter_inc_carry[0] = counter[0];
  for (genvar i = 1; i < CounterWidth; i++) begin : g_counter_inc_carry
    assign counter_inc_carry[i] = counter_inc_carry[i-1] & counter[i];
  end

  assign counter_upd[0] = ~counter[0];
  for (genvar i = 1; i < CounterWidth; i++) begin : g_counter_upd
    assign counter_upd[i] = counter[i] ^ counter_inc_carry[i-1];
  end
