...
module ibex_counter #(
  ...
  // Increment
  logic [CounterWidth:0] c;
  always_comb begin
    c[0] = 1'b1;
    for (int i = 0; i < CounterWidth; i = i + 1) begin
      if (i >= 3 && (i & 3) == 3) begin
        c[i+1] = c[i-3] & counter[i-3] & counter[i-2] & counter[i-1] & counter[i];
      end else begin
        c[i+1] = c[i] & counter[i];
      end
    end
  end
  assign counter_upd = counter[CounterWidth-1:0] ^ c[CounterWidth-1:0];
