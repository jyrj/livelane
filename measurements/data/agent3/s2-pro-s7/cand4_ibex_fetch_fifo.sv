...
`endif
  ...
  logic pop_fifo_cond;
  assign pop_fifo_cond = out_valid_o & (~aligned_is_compressed | out_addr_o[1]);
  ...
  for (genvar i = 0; i < (DEPTH - 1); i++) begin : g_fifo_next
    ...
    assign entry_en[i] = out_ready_i ? ((valid_pushed[i+1] & pop_fifo_cond) |
                         // a new request is incoming and this is the lowest free entry
                         (in_valid_i & lowest_free_entry[i] & ~pop_fifo_cond)) : (in_valid_i & lowest_free_entry[i]);
