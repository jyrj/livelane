...
module ibex_load_store_unit import ibex_pkg::*; import ibex_cheriot_pkg::*; #(
  ...
  logic        cap_mem_wdata_tag;
  logic [31:0] cap_mem_wdata_data;
  assign {cap_mem_wdata_tag, cap_mem_wdata_data} = cheriot_cap_to_mem(lsu_wcap_i);

  always_comb begin
    if ((BaseIsa == BaseIsaRV32IorCHERIoT) & (cheriot_enable_i == IbexMuBiOn) & lsu_is_cap_i) begin
      if (lsu_we_i & (ls_fsm_cs == CTX_WAIT_GNT2))
        {data_wdata_tag, data_wdata_data} = {cap_mem_wdata_tag, cap_mem_wdata_data};
      else if (lsu_we_i)
