...
module ibex_counter #(
  ...
  // Increment (parallel prefix Kogge-Stone carry lookahead)
  localparam int CntLevels = $clog2(CounterWidth) > 0 ? $clog2(CounterWidth) : 1;
  logic [CntLevels:0][CounterWidth-1:0] carry_prefix;

  assign carry_prefix[0] = counter[CounterWidth-1:0];
  for (genvar l = 0; l < CntLevels; l++) begin : g_prefix_level
    localparam int Offset = 1 << l;
    for (genvar i = 0; i < CounterWidth; i++) begin : g_prefix_bit
      if (i < Offset) begin : g_pass
        assign carry_prefix[l+1][i] = carry_prefix[l][i];
      end else begin : g_and
        assign carry_prefix[l+1][i] = carry_prefix[l][i] & carry_prefix[l][i - Offset];
      end
    end
  end

  logic [CounterWidth-1:0] counter_inc_carry;
  assign counter_inc_carry[0] = 1'b1;
  assign counter_upd[0]       = counter[0] ^ 1'b1;
  for (genvar g = 1; g < CounterWidth; g++) begin : g_counter_inc
    assign counter_inc_carry[g] = carry_prefix[CntLevels][g-1];
