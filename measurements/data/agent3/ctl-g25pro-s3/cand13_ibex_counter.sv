...
module ibex_counter #(
  ...
  // Update
  always_comb begin
    logic [63:0] counter_load_for_we;

    // This temporary variable computes the value for a write to the low part of the counter.
    // It is independent of counterh_we_i to break the reconvergent path.
    counter_load_for_we[63:32] = counter[63:32];
    counter_load_for_we[31:0]  = counter_val_i;

    // Original logic for we and counter_load must be preserved for correctness and to avoid
    // latches on unused outputs.
    ...
    // Restructured next value logic with a priority structure to improve timing.
    if (counterh_we_i) begin
      counter_d = counter[CounterWidth-1:0];
    end else if (counter_we_i) begin
      counter_d = counter_load_for_we[CounterWidth-1:0];
