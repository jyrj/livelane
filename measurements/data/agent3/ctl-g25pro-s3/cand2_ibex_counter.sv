...
module ibex_counter #(
  ...
  always_comb begin
    if (counterh_we_i) begin
      // Write high part. This has priority over counter_we_i as they are mutually exclusive.
      counter_d = counter[CounterWidth-1:0]; // Start with current value
      if (CounterWidth > 32) begin
        counter_d[CounterWidth-1:32] = counter_val_i;
      end
      // for CounterWidth <= 32, this is a no-op (hold), which is correct.
    end else if (counter_we_i) begin
      // Write low part.
      counter_d = counter[CounterWidth-1:0]; // Start with current value
      counter_d[31:0] = counter_val_i;
      // Truncation of counter_val_i is implicit if CounterWidth < 32.
