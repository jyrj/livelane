...
`endif
  ...
  for (genvar i = 0; i < DEPTH; i++) begin : g_fifo_regs
    if (ResetAll) begin : g_rdata_ra
      ...
      if (i < DEPTH-1) begin : g_less
        (* keep = 1 *) wire [31:0] rdata_pop = valid_pushed[i+1] ? rdata_d[i] : rdata_q[i];
        (* keep = 1 *) wire [31:0] rdata_nopop = (in_valid_i & lowest_free_entry[i]) ? rdata_d[i] : rdata_q[i];
        (* keep = 1 *) wire err_pop = valid_pushed[i+1] ? err_d[i] : err_q[i];
        (* keep = 1 *) wire err_nopop = (in_valid_i & lowest_free_entry[i]) ? err_d[i] : err_q[i];
        always_ff @(posedge clk_i) begin
          rdata_q[i] <= pop_fifo ? rdata_pop : rdata_nopop;
          err_q[i]   <= pop_fifo ? err_pop : err_nopop;
        end
