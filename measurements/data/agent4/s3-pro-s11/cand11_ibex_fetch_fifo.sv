...
`endif
  ...
  logic [DEPTH-1:0] valid_d_pop_1, valid_d_pop_0;
  always_comb begin
    for (int i = 0; i < DEPTH-1; i++) begin
      valid_d_pop_1[i] = valid_pushed[i+1] & ~clear_i;
      valid_d_pop_0[i] = valid_pushed[i] & ~clear_i;
    end
    valid_d_pop_1[DEPTH-1] = 1'b0;
    valid_d_pop_0[DEPTH-1] = valid_pushed[DEPTH-1] & ~clear_i;
  end

  logic pop_cond_uc1, pop_cond_uc0;
  assign pop_cond_uc1 = valid & (~aligned_is_compressed | out_addr_o[1]);
  assign pop_cond_uc0 = (out_addr_o[1] ? valid_unaligned : valid) & (~aligned_is_compressed | out_addr_o[1]);

  logic pop_uc1, pop_uc0;
  assign pop_uc1 = out_ready_i & pop_cond_uc1;
  assign pop_uc0 = out_ready_i & pop_cond_uc0;

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      valid_q <= '0;
    end else begin
      valid_q <= unaligned_is_compressed ? (pop_uc1 ? valid_d_pop_1 : valid_d_pop_0)
                                         : (pop_uc0 ? valid_d_pop_1 : valid_d_pop_0);
    end
  end
