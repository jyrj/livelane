...
`endif
  ...
  for (genvar i = 0; i < (DEPTH - 1); i++) begin : g_fifo_next
    ...
    assign valid_d[i] = (out_addr_o[1] ? (unaligned_is_compressed ? ((out_ready_i & valid) ? valid_pushed[i+1] : valid_pushed[i]) : ((out_ready_i & valid_unaligned) ? valid_pushed[i+1] : valid_pushed[i])) : valid_popped[i]) & ~clear_i;
