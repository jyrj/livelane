...
`endif
  ...
  for (genvar i = 0; i < (DEPTH - 1); i++) begin : g_fifo_next
    ...
    assign valid_pushed[i] = (in_valid_i & lowest_free_entry[i]) |
                             valid_q[i];
    wire pop_cond_i = out_valid_o & (~aligned_is_compressed | out_addr_o[1]);
    // Popping the FIFO shifts all entries down
    assign valid_popped[i] = out_ready_i ? (pop_cond_i ? valid_pushed[i+1] : valid_pushed[i]) : valid_pushed[i];
    // All entries are wiped out on a clear
    assign valid_d[i] = valid_popped[i] & ~clear_i;

    // data flops are enabled if there is new data to shift into it, or
    assign entry_en[i] = out_ready_i ? ((valid_pushed[i+1] & pop_cond_i) |
                         // a new request is incoming and this is the lowest free entry
                         (in_valid_i & lowest_free_entry[i] & ~pop_cond_i)) :
