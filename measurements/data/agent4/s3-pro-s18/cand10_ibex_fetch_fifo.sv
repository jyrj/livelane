...
`endif
  ...
  wire pop_fifo_err0 = out_ready_i & (out_addr_o[1] ? ((cheriot_force_uc_i | (rdata[17:16] != 2'b11)) ? valid : valid_unaligned) : (valid & (rdata[1:0] == 2'b11)));
  wire pop_fifo_err1 = out_ready_i & (out_addr_o[1] ? (cheriot_force_uc_i ? valid : valid_unaligned) : valid);
  // Since an entry can contain unaligned instructions, popping an entry can leave the entry valid
  assign pop_fifo = err ? pop_fifo_err1 : pop_fifo_err0;
  ...
  for (genvar i = 0; i < (DEPTH - 1); i++) begin : g_fifo_next
    ...
    assign entry_en[i] = pop_fifo ? valid_pushed[i+1] : (in_valid_i & lowest_free_entry[i]);
