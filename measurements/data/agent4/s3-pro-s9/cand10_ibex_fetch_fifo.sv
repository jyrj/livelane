...
`endif
  ...
  logic pop_cond;
  assign pop_cond = out_valid_o & (~aligned_is_compressed | out_addr_o[1]);
  assign pop_fifo = out_ready_i & pop_cond;
  ...
  for (genvar i = 0; i < (DEPTH - 1); i++) begin : g_fifo_next
    ...
    assign valid_popped[i] = out_ready_i ? (pop_cond ? valid_pushed[i+1] : valid_pushed[i]) : valid_pushed[i];
    // All entries are wiped out on a clear
    assign valid_d[i] = valid_popped[i] & ~clear_i;

    // data flops are enabled if there is new data to shift into it, or
    assign entry_en[i] = out_ready_i ? (pop_cond ? valid_pushed[i+1] : (in_valid_i & lowest_free_entry[i])) : (in_valid_i & lowest_free_entry[i]);
