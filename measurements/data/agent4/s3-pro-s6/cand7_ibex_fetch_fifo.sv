...
`endif
  ...
  for (genvar i = 0; i < (DEPTH - 1); i++) begin : g_fifo_next
    ...
    assign entry_en[i] = out_ready_i ? ((out_valid_o & (~aligned_is_compressed | out_addr_o[1])) ? valid_pushed[i+1] : (in_valid_i & lowest_free_entry[i])) : (in_valid_i & lowest_free_entry[i]);
