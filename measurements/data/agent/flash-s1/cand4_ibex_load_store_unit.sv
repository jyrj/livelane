...
module ibex_load_store_unit import ibex_pkg::*; import ibex_cheriot_pkg::*; #(
  ...
  always_comb begin
    if ((BaseIsa == BaseIsaRV32IorCHERIoT) & (cheriot_enable_i == IbexMuBiOn) & lsu_is_cap_i)
      ...
      if (lsu_type_i == 2'b00) begin // Writing a word
        if (!handle_misaligned_q) begin // first part of potentially misaligned transaction
          data_be = (4'b1111 >> data_offset);
        end else begin // second part of misaligned transaction
          data_be = {1'b0, data_offset[1] && data_offset[0], data_offset[1] || data_offset[0], data_offset[0]};
        end
      end else if (lsu_type_i == 2'b01) begin // Writing a half word
        if (!handle_misaligned_q) begin // first part of potentially misaligned transaction
          data_be = (4'b0011 << data_offset);
        end else begin // second part of misaligned transaction
          data_be = 4'b0001;
        end
      end else begin // lsu_type_i == 2'b10 or 2'b11: Writing a byte
        data_be = (4'b0001 << data_offset);
      end
    end  // if lsu_cap
  end
