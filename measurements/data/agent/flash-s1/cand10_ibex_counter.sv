...
module ibex_counter #(
  ...
  // Update
  always_comb begin
    logic [CounterWidth-1:0] counter_upd_val;
    logic [63:0]             counter_load_val;
    logic                    we_val;

    counter_upd_val = counter[CounterWidth-1:0] + {{CounterWidth - 1{1'b0}}, 1'b1};

    // Write
    we_val = counter_we_i | counterh_we_i;
    counter_load_val = counterh_we_i ? {counter_val_i, counter[31:0]} : {counter[63:32], counter_val_i};

    // Next value logic
    if (we_val) begin
      counter_d = counter_load_val[CounterWidth-1:0];
    end else if (counter_inc_i) begin
      counter_d = counter_upd_val;
    end else begin
