...
module ibex_load_store_unit import ibex_pkg::*; import ibex_cheriot_pkg::*; #(
  ...
  // Added temporary signals to flatten rdata_h_ext logic
  logic [15:0] rdata_h_unextended;
  logic        rdata_h_sign_bit_val;

  // Stage 1: Extract the 16-bit half-word and identify its sign bit using a single continuous assignment
  assign {rdata_h_sign_bit_val, rdata_h_unextended} =
    (rdata_offset_q == 2'b00) ? {data_rdata_i[15], data_rdata_i[15:0]} :
    (rdata_offset_q == 2'b01) ? {data_rdata_i[23], data_rdata_i[23:8]} :
    (rdata_offset_q == 2'b10) ? {data_rdata_i[31], data_rdata_i[31:16]} :
    (rdata_offset_q == 2'b11) ? {data_rdata_i[7], {rdata_q[31:24], data_rdata_i[7:0]}} :
                                {data_rdata_i[15], data_rdata_i[15:0]}; // Default behavior matches 2'b00

  // Stage 2: Apply sign or zero extension based on data_sign_ext_q using a continuous assignment
  assign rdata_h_ext = data_sign_ext_q ? {{16{rdata_h_sign_bit_val}}, rdata_h_unextended} :
                                         {16'h0000, rdata_h_unextended};
