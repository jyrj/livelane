...
module ibex_load_store_unit import ibex_pkg::*; import ibex_cheriot_pkg::*; #(
  ...
  logic [15:0] selected_half_word;
  logic        selected_sign_bit;

  // Select the 16-bit half-word value based on rdata_offset_q
  assign selected_half_word = (rdata_offset_q == 2'b00) ? data_rdata_i[15:0] :
                              (rdata_offset_q == 2'b01) ? data_rdata_i[23:8] :
                              (rdata_offset_q == 2'b10) ? data_rdata_i[31:16] :
                              /* 2'b11 */                {rdata_q[31:24], data_rdata_i[7:0]};

  // Select the sign bit for the half-word based on rdata_offset_q, preserving original behavior
  assign selected_sign_bit = (rdata_offset_q == 2'b00) ? data_rdata_i[15] :
                             (rdata_offset_q == 2'b01) ? data_rdata_i[23] :
                             (rdata_offset_q == 2'b10) ? data_rdata_i[31] :
                             /* 2'b11 */                data_rdata_i[7];

  // Apply sign/zero extension based on data_sign_ext_q
  assign rdata_h_ext = data_sign_ext_q ? {{16{selected_sign_bit}}, selected_half_word} :
                                         {16'h0000, selected_half_word};
