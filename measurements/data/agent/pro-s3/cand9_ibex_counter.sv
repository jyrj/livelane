...
module ibex_counter #(
  ...
  // Increment
  logic [CounterWidth:0] c_carry;
  assign c_carry[0] = 1'b1;
  genvar inc_idx;
  generate
    for (inc_idx = 0; inc_idx < CounterWidth; inc_idx++) begin : g_inc
      assign counter_upd[inc_idx] = counter[inc_idx] ^ c_carry[inc_idx];
      assign c_carry[inc_idx+1] = counter[inc_idx] & c_carry[inc_idx];
    end
  endgenerate
