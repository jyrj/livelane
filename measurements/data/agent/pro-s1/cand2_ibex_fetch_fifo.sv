...
`endif
  ...
  assign pop_fifo = out_ready_i & (in_valid_i ? (out_addr_o[1] ? (unaligned_is_compressed ? 1'b1 : (valid_q[1] | valid_q[0])) : ~aligned_is_compressed) : (out_addr_o[1] ? (unaligned_is_compressed ? valid_q[0] : valid_q[1]) : (valid_q[0] & ~aligned_is_compressed)));
