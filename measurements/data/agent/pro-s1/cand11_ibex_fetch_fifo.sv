...
`endif
  ...
  logic pop_fifo_uc1_cond, pop_fifo_uc0_cond, pop_fifo_cond;
  assign pop_fifo_uc1_cond = valid & (~aligned_is_compressed | out_addr_o[1]);
  assign pop_fifo_uc0_cond = (out_addr_o[1] ? valid_unaligned : valid) & (~aligned_is_compressed | out_addr_o[1]);
  assign pop_fifo_cond = unaligned_is_compressed ? pop_fifo_uc1_cond : pop_fifo_uc0_cond;
  assign pop_fifo = out_ready_i & pop_fifo_cond;
  ...
  for (genvar i = 0; i < (DEPTH - 1); i++) begin : g_fifo_next
    ...
    assign valid_popped[i] = out_ready_i ? (pop_fifo_cond ? valid_pushed[i+1] : valid_pushed[i]) : valid_pushed[i];
    // All entries are wiped out on a clear
    assign valid_d[i] = valid_popped[i] & ~clear_i;

    // data flops are enabled if there is new data to shift into it, or
    assign entry_en[i] = out_ready_i ? (pop_fifo_cond ? valid_pushed[i+1] : (in_valid_i & lowest_free_entry[i])) :
                         // a new request is incoming and this is the lowest free entry
                         (in_valid_i & lowest_free_entry[i]);
  ...
  assign valid_popped     [DEPTH-1] = out_ready_i ? (pop_fifo_cond ? 1'b0 : valid_pushed[DEPTH-1]) : valid_pushed[DEPTH-1];
